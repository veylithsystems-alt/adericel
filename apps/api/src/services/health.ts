...
export async function collectHealth(
  ...
  components.push(await check('database', async () => {
  ...
  components.push(await check('object-storage', async () => {
  ...
  components.push(await check('event-outbox', async () => {
  ...
  components.push(await check('integrations', async () => {
  ...
  components.push(await check('scheduled-jobs', async () => {
    const row = await app.db.withPlatform(async (ctx) =>
    ...
    if (overdue > 0) {
      platformProblems.push(`${overdue} scheduled job(s) are overdue. The worker may not be running.`);
    }
    return {
  ...
  components.push(await check('action-execution', async () => {
    const row = await app.db.withPlatform(async (ctx) =>
