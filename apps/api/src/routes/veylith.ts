...
import { z } from 'zod';
import { compileAutonomyPolicy, type OperationRiskClass } from '@adericel/autonomy';
import {
  ...
  server.get(
    ...
    async (request, reply) => {
      ...
      const { exceptions, overdue } = await app.db.withPlatform(async (ctx) => {
        ...
        return {
          exceptions: query.overdueOnly
            ? await queue.overdue()
            : await queue.open({ limit: query.limit }),
